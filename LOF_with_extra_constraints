...
LEFT JOIN gender_concept_filter sex_concept
    ...
    # Run model
    df_results, anomalies, scores = local_outlier_factor(in_range, X_scaled)
...
def local_outlier_factor(df, X_scaled):
    ...
    scores = -lof.negative_outlier_factor_  # invert for easier interpretation (higher = more anomalous)
...
def graph_distribution(measurement, df, anomalies):
    ...
    plt.title("Anomaly Detection (LOF - Full Dataset)")
...
def graph_anomalies(measurement, scores):
    ...
    plt.title(f"{measurement}LOF Decision Score Distribution")
