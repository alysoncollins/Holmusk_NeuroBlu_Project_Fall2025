...
def main():
    ...
    full_df = pl.DataFrame(values)
    if measurement == 'body_mass_index':
        df = full_df.unique(subset=["value_as_number", "age", "sex"])
    else:
        df = full_df.unique(subset=["value_as_number", "age", "sex", "bmi"])
    ...
    # Run model
    df_results, anomalies, scores = local_outlier_factor(df, X_scaled)


    # Filter full dataset to only anomaly rows
    if measurement == 'body_mass_index':
        key_cols = ["value_as_number", "age", "sex"]
    else:
        key_cols = ["value_as_number", "age", "sex", "bmi"]

    filtered_full_df = full_df.join(anomalies.select(key_cols), on=key_cols, how="semi")
    if measurement == 'body_mass_index':
        data = filtered_full_df.select(["value_as_number", "age", "sex"])
    else:
        data = filtered_full_df.select(["value_as_number", "age", "sex", "bmi"])
    neuroblu.save_df(filtered_full_df.to_pandas(), f"{measurement}_filtered_full_dataset", overwrite=True)
    ...
    if selector == 1:
        graph_distribution_bmi_only(df_results, data, percentage)
    else:
        graph_distribution(measurement, df, data, percentage)



def local_outlier_factor(df, X_scaled):
...
def graph_distribution(measurement, df, anomalies, percentage):
    ...
    plt.hexbin(anomalies["value_as_number"], anomalies["age"], gridsize=50, cmap="Blues", mincnt=1)
    ...
    # BMI graph (hexbin)
    plt.figure(figsize=(10, 6))
    plt.hexbin(anomalies["value_as_number"], anomalies["bmi"], gridsize=50, cmap="Blues", mincnt=1)
    plt.colorbar(label="Count")
